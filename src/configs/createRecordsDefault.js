...
module.exports = {
    ...
    createRecordsDefault: async () => {
        try {
            ...
            if (state3 == null) await Order_State.create({ state_id: 3, state_name: 'Shipping' })
            ...
            if (state5 == null) await Order_State.create({ state_id: 5, state_name: 'Cancelled' })

            state6 = await Order_State.findOne({ where: { state_id: 6 } })
            if (state6 == null) await Order_State.create({ state_id: 6, state_name: 'Cancelled By Shop' })
        } catch (err) {
            console.log(err)
        }
    }
